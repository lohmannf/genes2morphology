...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        raw_shape: list,              # Shape of the raw image data (NCHW).
        ...
        if mask is not None:
            self._raw_idx = self._raw_idx[mask]
    ...
    def set_dyn_len(self, new_len: int) -> int:
        self._raw_idx = self._base_raw_idx[:new_len]

    def set_classes(self, cls_list: list) -> None:
    ...
    def _load_raw_labels(self) -> None: # to be overridden by subclass
        raise NotImplementedError
    ...
    def get_details(self, idx: int) -> dnnlib.EasyDict:
        d = dnnlib.EasyDict()
    ...
    def label_shape(self) -> list:
        if self._label_shape is None:
            ...
            if isinstance(raw_labels, dict):
                self._label_shape = [1]  # TODO: dict = zip dataset, fix this
    ...
    @property
    def labels_are_genes(self) -> bool:
        return hasattr(self, "gex")
...
class ImageFolderDataset(Dataset):
    def __init__(
        ...
        resolution: Optional[int] = None, # Ensure specific resolution, None = highest available.
    ...
    def _open_file(self, fname: str) -> Optional[IO]:
        if self._type == 'dir':
...
class ImageGeneDataset(Dataset):
    def __init__(
        ...
        **super_kwargs,                      # Additional arguments for the Dataset base class.
        ...
        self.gex = self._load_raw_labels()
    ...
    def _get_raw_labels(self) -> sparse.csr_matrix:
        return self.gex
    
    def _load_raw_labels(self) -> np.ndarray:
        gex = sc.read_h5ad(self.gex_path)
        sc.pp.normalize_total(gex)
        sc.pp.log1p(gex)
        return gex.X

    def get_label(self, idx: int) -> np.ndarray:
        label = self.gex[self._raw_idx[idx]].toarray().flatten()
        return label.copy()
    ...
    def get_details(self, idx: int) -> dnnlib.EasyDict:
        ...
        d.raw_idx = int(self._raw_idx[idx])
        d.xflip = (int(self._xflip[idx]) != 0)
        d.raw_label = self.gex[d.raw_idx].toarray().copy()
        return d

class ImageDummyGeneDataset(Dataset):
    def __init__(
        self,
        path: str,                           # Path to directory or zip.
        gex_path: str,
    ...
    def get_label(self, idx: int) -> np.ndarray:
        label = self.gex[self._raw_idx[idx]]
        return label.copy()
