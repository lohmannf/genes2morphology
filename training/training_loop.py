...
def setup_snapshot_image_grid(
    ...
    elif training_set.labels_are_text or training_set.labels_are_genes:
        all_indices = list(range(len(training_set)))
        grid_indices = [all_indices[i % len(all_indices)] for i in range(gw * gh)]
...
def save_image_grid(
    ...
    if log_wandb:
        wandb.log({"samples": wandb.Image(img, caption="Different latent samples")})
...
def save_prompts_dict(
    ...
    if log_wandb:
        wandb.log({log_key: [wandb.Image(img, caption=f"{prompt}, Left: GT, Top: psi=1.0, Bottom: psi=0.0") for prompt, img in prompts_dict.items()]})
...
def save_samples(
    ...
    device: torch.device,
    suffix: str,
    prompts: Union[list[str], torch.Tensor] = None,
    embs: torch.Tensor = None,
    n_samples: int = 5,
    gt_imgs: torch.Tensor = None,
    log_wandb: bool = False,
    log_key: str = "samples"
) -> None:
    ...
    if G_ema.c_dim > 0:
        ...
        if embs is not None:
            assert embs.shape[1] == G_ema.c_dim
            embs = embs.to(torch.float32).to(device)

        prompts_results = dict()
        if gt_imgs is not None:
            assert len(gt_imgs) == len(prompts)
            # rescale ground truth to -1, 1
            gt_imgs = gt_imgs * (2 / 255.) - 1
            gt_imgs = torch.clamp(gt_imgs, -1,1)
        for i,prompt in enumerate(prompts):
            pts = [prompt]*len(z)
            if isinstance(prompt, torch.Tensor):
                pts = torch.stack(pts).to(device)
            ws = [
                G_ema.mapping(z, pts, truncation_psi=1.0),
                G_ema.mapping(z, pts, truncation_psi=0.5),
            ]
            imgs = []
            for j, wi in enumerate(ws):
                if embs is not None:
                    wi[..., G_ema.z_dim:] = embs[i]
                samp = G_ema.synthesis(wi, noise_mode="const").cpu()
                if gt_imgs is not None:
                    gt_curr = gt_imgs[[i]] if j == 0 else torch.ones_like(gt_imgs[0])[None]
                    samp = torch.cat((gt_curr, samp))
                imgs.append(samp)

            lab = prompt if isinstance(prompt, str) else i
            prompts_results[lab] = save_image_grid(torch.cat(imgs), drange=[-1, 1], grid_size=(n_samples+int(gt_imgs is not None), -1))
        save_prompts_dict(prompts_results, path=save_path, log_wandb=log_wandb, log_key=log_key)

    else:
        z = np.random.RandomState(0).randn(n_samples**2, G_ema.z_dim)
        z = torch.from_numpy(z).float().to(device)
        imgs = G_ema(z, c=None, noise_mode="const").cpu().numpy()
        save_image_grid(imgs, drange=[-1,1], grid_size=(n_samples, n_samples), fname=save_path, log_wandb=log_wandb)


def network_summaries(G: nn.Module, D: nn.Module, device: torch.device) -> None:
    z = torch.randn([2, G.z_dim], device=device)
    c = torch.randn([2, G.c_dim], device=device)
...
def get_diverse_set(group_mask: np.ndarray, n_samples_per_cluster: int, dataset_kwargs: dict):
    '''Get same number of samples from each group in group_mask'''
    imgs = []
    prompts = []
    for l in np.unique(group_mask):
        dataset_kwargs.mask = group_mask == l
        ds = dnnlib.util.construct_class_by_name(**dataset_kwargs)
        i, p = next(iter(torch.utils.data.DataLoader(dataset=ds, batch_size=n_samples_per_cluster, shuffle=True)))
        imgs.append(i)
        prompts.append(p)

    return torch.cat(imgs, dim=0), torch.cat(prompts, dim=0)
...
def training_loop(
    ...
    G_scheduler_kwargs      = {},        # Options for learning rate schedulers
    D_scheduler_kwargs      = {},        # Options for learning rate schedulers
    ...
    dist.print0('Setting up validation...')
    n_samples = 10
    if (dist.get_rank() == 0) and training_set.labels_are_genes:
        if val_clusters is None:
            validation_set = dnnlib.util.construct_class_by_name(**validation_set_kwargs)
            val_imgs, val_prompts = next(iter(torch.utils.data.DataLoader(dataset=validation_set, batch_size=n_samples, shuffle=True)))
        else:
            val_clusters = np.load(val_clusters)
            val_imgs, val_prompts = get_diverse_set(val_clusters, max(1,n_samples//len(np.unique(val_clusters))), dataset_kwargs = validation_set_kwargs)
        if train_clusters is None:
            train_imgs, train_prompts  = next(iter(torch.utils.data.DataLoader(dataset=training_set, batch_size=n_samples, shuffle=True)))
        else:
            train_clusters = np.load(train_clusters)
            train_imgs, train_prompts = get_diverse_set(train_clusters, max(1,n_samples//len(np.unique(train_clusters))), training_set_kwargs)

        if embedding_prompts is not None:
            embedding_prompts = torch.from_numpy(np.load(embedding_prompts)).to(torch.float32)
    ...
    if training_set.labels_are_genes:
        G_kwargs.c_input_dim = training_set.label_dim
    ...
    for name, module, opt_kwargs, scheduler_kwargs in [('D', D, D_opt_kwargs, D_scheduler_kwargs), ('G', G, G_opt_kwargs, G_scheduler_kwargs)]:
        ...
        phases += [dnnlib.EasyDict(name=name, module=module, opt=opt, interval=1, 
                                   lr_scheduler=dnnlib.util.construct_class_by_name(optimizer = opt, **scheduler_kwargs) if scheduler_kwargs else None)]
    ...
    if dist.get_rank() == 0:
        ...
        save_image_grid(images, drange=[0, 255], grid_size=grid_size, fname=os.path.join(run_dir, "reals.png"), log_wandb=False)
    ...
    while True:
        ...
        if (dist.get_rank() == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            save_samples(G_ema, run_dir=run_dir, device=device, suffix=f'{cur_nimg//1000:06d}_val', prompts=val_prompts, 
                         gt_imgs=val_imgs, n_samples=n_samples, log_wandb=True, log_key="samples/validation")
            if train_prompts is not None:
                save_samples(G_ema, run_dir=run_dir, device=device, suffix=f'{cur_nimg//1000:06d}_train', prompts=train_prompts.float(), 
                            gt_imgs=train_imgs, n_samples=n_samples, log_wandb=True, log_key="samples/training")
                
            if embedding_prompts is not None:
                save_samples(G_ema, run_dir=run_dir, device=device, suffix=f'{cur_nimg//1000:06d}_avg', prompts=torch.zeros((len(embedding_prompts), val_prompts.shape[1]), dtype=torch.float32), 
                            gt_imgs=None, n_samples=n_samples, log_wandb=True, log_key="samples/clusters", embs=embedding_prompts)
        ...
        for phase in phases:
            ...
            if phase.lr_scheduler is not None:
                training_stats.report0(f'Parameters/{phase.name}/lr', phase.lr_scheduler.get_last_lr()[0])
        ...
        if stats_jsonl is not None:
            ...
            stats_jsonl.flush()
        if stats_tfevents is not None and stats_wandb is not None:
            global_step = int(cur_nimg / 1e3)
            walltime = timestamp - start_time
            for name, value in stats_dict.items():
                stats_tfevents.add_scalar(name, value.mean, global_step=global_step, walltime=walltime)
                stats_wandb.log({name: value.mean})
            for name, value in stats_metrics.items():
                stats_tfevents.add_scalar(f'Metrics/{name}', value, global_step=global_step, walltime=walltime)
                stats_wandb.log({f'Metrics/{name}': value})
        ...
        if loss.scheduler:
            loss.scheduler.step()
        for phase in phases:
            if phase.lr_scheduler:
                phase.lr_scheduler.step()
        if done:
            break
