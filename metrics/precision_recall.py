...
def compute_distances(row_features: torch.Tensor, col_features: torch.Tensor, num_gpus: int, rank: int, col_batch_size: int) -> Optional[torch.Tensor]:
    ...
    for col_batch in col_batches[rank :: num_gpus]:
        dist_batch = torch.cdist(row_features.unsqueeze(0).float(), col_batch.unsqueeze(0).float())[0]
...
def compute_pr(opts, max_real: int, num_gen: int, nhood_size: int, row_batch_size: int, col_batch_size: int) -> tuple[float, float]:
    ...
    max_real = max_real // opts.num_gpus

    real_features = metric_utils.compute_feature_stats_for_dataset(
        opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
        rel_lo=0, rel_hi=0, capture_all=True, max_items=None, shuffle_size=max_real).get_all_torch().to(torch.float16).to(opts.device)
    ...
    for name, manifold, probes in [('precision', real_features, gen_features), ('recall', gen_features, real_features)]:
        ...
        for manifold_batch in tqdm(manifold.split(row_batch_size), disable=not (opts.rank==0)):
        ...
        pred = []
        for probes_batch in tqdm(probes.split(row_batch_size), disable=not (opts.rank==0)):
