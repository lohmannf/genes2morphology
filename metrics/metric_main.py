...
#from metrics import cell_count
from metrics import contrastive_ce
...
def fid50k_full(opts):
    ...
    is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
    assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=50000)
    return dict(fid50k_full=fid)

@register_metric
def fgd50k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
    assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
    fid = frechet_inception_distance.compute_fhd(opts, max_real=None, num_gen=50000, model="gigapath")
    return dict(fgd50k_full=fid)

@register_metric
def check_fgd50k(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
    assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
    fid = frechet_inception_distance.fhd_sanity_check(opts, max_real=None, model="gigapath")
    return dict(fgd50k_full=fid)

@register_metric
def check_fid50k(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
    assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
    fid = frechet_inception_distance.fhd_sanity_check(opts, max_real=None, model="inception")
    return dict(fgd50k_full=fid)

# @register_metric
# def cellcount_10k(opts):
#     opts.dataset_kwargs.update(max_size=10000, xflip=False)
#     opts.max_items = 10000
#     is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
#     assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
#     count_mse = cell_count.compute_cell_count_diff(opts)
#     return dict(cellcount_10k=count_mse)
...
def cesoft_10k(opts):
    ...
    is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
    assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
    ce = contrastive_ce.compute_cross_entropy(opts, soft_target=True)
    return dict(ce_10k=ce)
...
def cehard_10k(opts):
    ...
    is_imfolder = opts.dataset_kwargs.class_name == 'training.data_zip.ImageFolderDataset' or opts.dataset_kwargs.class_name == 'training.data_zip.ImageGeneDataset'
    assert is_imfolder, f'Calculating metrics on {opts.dataset_kwargs.class_name} are not supported'
    ce = contrastive_ce.compute_cross_entropy(opts, soft_target=False)
    return dict(ce_10k=ce)
