...
def get_feature_detector(
    ...
    if key not in _feature_detector_cache:
        ...
        if url.startswith("hist"):
            _feature_detector_cache[key] = torch.jit.load(url.split(":")[1]).to(device)
        else:
...
class FeatureStats:
    def __init__(self, capture_all: bool = False, capture_mean_cov: bool = False, max_items: bool = None, capture_mean: bool = False):
        self.capture_all = capture_all
        self.capture_mean_cov = capture_mean_cov
        self.capture_mean = capture_mean or capture_mean_cov
    ...
    **stats_kwargs,
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            c = next(c_iter).to(torch.float32)
