...
from diffusion_utils.pipeline import CustomStableDiffusionPipeline
...
def log_validation(args, accelerator, weight_dtype, step, ckpt_path, val_prompts):
    ...
    if args.pretrained_vae_path != ckpt_path:
        ...
        shutil.copyfile(args.preprocessor_config_file,
    ...
    feature_extractor = AutoFeatureExtractor.from_pretrained(args.preprocessor_config_file)#ckpt_path + "/feature_extractor")
    ...
    for i in range(len(val_prompts)):
        for j in range(n_imgs):
            with torch.autocast("cuda"):
                image = pipeline(
                    prompt_embeds=pad_prompts(val_prompts[i]),
                    generator=generator,
                    height=args.resolution,
                    width=args.resolution,
                    guidance_scale=args.guidance_scale,
                    negative_prompt_embeds=uncond_embedding).images[0]
            images.append(image)
            updated_validation_prompts.append('prompt'+str(i)+'-'+str(j))
            ...
            tracker.log(
                {
                    "validation": [
                        wandb.Image(
                            image,
                            caption=f"{i}: prompt {i//n_imgs} sample {i%n_imgs} - step {step}",)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("The path to prompt embeddings as numpy file."),
    )
    parser.add_argument(
        "--dataset_config_name",
        type=str,
    ...
    parser.add_argument(
        ...
        help=("A set of perturbation ids evaluated every `--validation_steps` and logged to `--report_to`."),
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--classifier_free_guidance", action="store_true", help="Whether or not to use classifier-free guidance."
    )
    parser.add_argument(
        "--p_uncond",
        type=float,
        default=0.1,
        help=(
            "Probability of discarding condition during training"
    ...
    parser.add_argument(
        ...
        default=1.5,
        help=(
            "Guidance weight at inference time, 1 disables guidance"
            " ignored if `--classifier_free_guidance` is not set."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to shuffle the prompts randomly before training."
        ),
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to the feature extractor config file"
        )
    )
...
def pad_prompts(embedding, no_batch_dim=False):
    ...
    Args:
        gene_embedding (tensor): gene embedding (embedding_dim,)
    ...
    if no_batch_dim:
        ...
        # replicate padded_tensor to have dimension (1, 77, 768)
...
def get_prompt(identifier):
    ...
    ----------
    identifier: int
        Row index for extracting prompt encoding

    Returns
    -------
    prompt_embeds: torch.Tensor
        Gex prompt embedding
    """
    global dataset_embs
    embeds = pad_prompts(torch.from_numpy(dataset_embs[identifier]).float(), True)
    return embeds
...
def main():
    ...
    global dataset_embs
    dataset_embs = np.load(args.dataset_embs)

    if args.shuffle_prompts:
        # randomly shuffle the prompt embeddings to train a baseline model
        random.seed(args.seed)
        idx = np.arange(len(dataset_embs), dtype=int)
        random.shuffle(idx)
        dataset_embs = dataset_embs[idx]

    validation_prompts = torch.from_numpy(np.load(args.validation_prompts)).float()
    ...
    if args.classifier_free_guidance:
        uncond_embedding = pad_prompts(torch.from_numpy(np.load(args.uncond_embedding)).float(), no_batch_dim=True)
    else:
        uncond_embedding = None
    ...
    def collate_fn(examples):
        ...
        if args.classifier_free_guidance and torch.rand(1) < args.p_uncond:
            prompt_embeds = [uncond_embedding] * len(examples)
        else:
            prompt_embeds = [get_prompt(example[caption_column]) for example in examples]
        input_ids = torch.stack(prompt_embeds)
    ...
    if args.classifier_free_guidance:
        logger.info(f"  Using Classifier-free guidance with w = {args.guidance_scale}")
    global_step = 0
    first_epoch = 0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                #print(encoder_hidden_states.size(), flush=True)
                ...
                if (global_step % args.checkpointing_steps == 0) | (global_step == args.max_train_steps):
                    if accelerator.is_main_process:
                        ...
                        if accelerator.is_main_process:
                            ...
                            feature_extractor = AutoFeatureExtractor.from_pretrained(args.preprocessor_config_file)
                                #args.pretrained_vae_path + '/feature_extractor/preprocessor_config.json')
                            ...
                            if (args.validation_prompts is not None) and \
                                ((global_step % args.validation_steps == 0) | (global_step == args.max_train_steps)):
