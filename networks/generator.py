...
from networks.mlp import MLPEncoder
...
class MappingNetwork(torch.nn.Module):
    def __init__(
        ...
        c_input_dim: int = None,      # Input dimension of condition
        c_architecture: str = "mlp"   # Type of conditional encoder: "mlp", "clip"
    ):
        super().__init__()
        self.z_dim = z_dim
        ...
        if conditional:
            if c_architecture == "clip":
                self.clip = CLIP(ckpt_file=CLIP_PATH)
                self.c_dim = self.clip.latent_dim
                self.c_encoder = None
                del self.clip.img_encoder
            elif c_architecture == "mlp":
                self.c_encoder = MLPEncoder(c_input_dim, 768, [1024], 0.4, 0.5)
                self.c_dim = self.c_encoder.latent_dim
                self.clip = None
        else:
            self.c_dim = 0
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        c_input_dim: int = None,
        c_architecture: str = "mlp",
        ...
        self.mapping = MappingNetwork(z_dim=z_dim, conditional=conditional, c_input_dim=c_input_dim, c_architecture=c_architecture)
        ...
        if train_mode == 'all':
            self.trainable_layers = ['synthesis', 'mapping.mlp', 'c_encoder']
        elif train_mode == 'text_encoder':
            self.trainable_layers = ['c_encoder', 'clip']
        elif train_mode == 'freeze8':
            self.trainable_layers = [f"b{x}" for x in self.synthesis.block_resolutions if x > 8]
